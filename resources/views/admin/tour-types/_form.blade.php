@php $editing = isset($tourType) && $tourType->exists; @endphp
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-6">
        <div class="bg-white rounded-xl shadow-sm p-6 space-y-5">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                          placeholder="Brief description of this tour type…"
                          class="w-full px-4 py-3 border border-gray-300 rounded-lg bg-white focus:ring-2 focus:ring-[#FEBC11] focus:border-[#FEBC11] text-sm">{{ old('description', $tourType->description ?? '') }}</textarea>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            @include('admin.partials.seo-panel', ['model' => $tourType ?? null])
        </div>
    </div>
    ...
    <div class="space-y-6">
        <div class="bg-white rounded-xl shadow-sm p-6">
            ...
            @include('admin.media.picker', [
                'name'  => 'featured_image',
                'value' => old('featured_image', $tourType->featured_image ?? ''),
        ...
        <div class="flex items-center gap-3">
            <button type="submit" class="w-full px-6 py-2.5 bg-[#FEBC11] text-[#131414] text-sm font-semibold rounded-lg hover:bg-yellow-400 transition shadow-sm">
                {{ $editing ? 'Update Tour Type' : 'Create Tour Type' }}
            </button>
        </div>
    </div>
</div>
