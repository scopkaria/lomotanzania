<x-app-layout>
    <x-slot name="header">Edit: {{ $tourType->name }}</x-slot>
    ...
    <div class="mb-4 flex items-center justify-between">
        ...
        <a href="{{ route('tour-types.show', ['locale' => app()->getLocale(), 'slug' => $tourType->slug]) }}" target="_blank" class="inline-flex items-center gap-1.5 px-4 py-2 bg-brand-green text-white text-xs font-semibold rounded-lg hover:bg-green-800 transition shadow-sm">
    ...
    <form action="{{ route('admin.tour-types.update', $tourType) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
