<div class="max-w-2xl space-y-6" x-data="countryLocationSearch()">
        ...
        <div class="relative">
            <div class="relative">
                ...
                <input type="text" x-model="query" @input.debounce.300ms="search()" @focus="showDropdown = results.length > 0"
                       ...
                       placeholder="Search country (e.g. Tanzania, Kenya)"
    ...
    <div class="bg-white rounded-xl shadow-sm p-6">
        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="slug" id="slug" value="{{ old('slug', $country->slug ?? '') }}"
                       class="w-full px-4 py-3 text-sm border border-gray-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-[#FEBC11] focus:border-[#FEBC11]"
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                          class="w-full px-4 py-3 text-sm border border-gray-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-[#FEBC11] focus:border-[#FEBC11]">{{ old('description', $country->description ?? '') }}</textarea>
    ...
    <div class="bg-white rounded-xl shadow-sm p-6">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" step="any" name="latitude" id="latitude" value="{{ old('latitude', $country->latitude ?? '') }}"
                       class="w-full px-4 py-3 text-sm border border-gray-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-[#FEBC11] focus:border-[#FEBC11]"
                       placeholder="-6.3690">
            ...
            <div>
                ...
                <input type="number" step="any" name="longitude" id="longitude" value="{{ old('longitude', $country->longitude ?? '') }}"
                       class="w-full px-4 py-3 text-sm border border-gray-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-[#FEBC11] focus:border-[#FEBC11]"
                       placeholder="34.8888">
    ...
    <div class="bg-white rounded-xl shadow-sm p-6">
        @include('admin.media.picker', [
            'name'  => 'featured_image',
            'value' => old('featured_image', $country->featured_image ?? ''),
            'label' => 'Featured Image',
        ])
    </div>

    {{-- SEO Settings --}}
    @include('admin.partials.seo-panel', ['model' => $country ?? null])
    ...
    <div class="flex items-center gap-3">
        ...
        <a href="{{ route('admin.countries.index') }}" class="px-5 py-2.5 text-sm text-gray-500 hover:text-gray-700">Cancel</a>
    </div>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('countryLocationSearch', () => ({
        query: '',
        results: [],
        loading: false,
        showDropdown: false,
        ...
        async search() {
            ...
            try {
                const url = `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(q)}.json?access_token=${MAPBOX_TOKEN}&limit=5&types=country,region`;
                console.log('[Mapbox] Country search:', q);
