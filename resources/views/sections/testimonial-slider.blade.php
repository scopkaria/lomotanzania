        ...
        <div class="relative overflow-hidden">
            ...
            <div x-show="current === {{ $i }}"
                ...
                <div class="flex items-center justify-center gap-1 mb-6">
                    @for($s = 0; $s < 5; $s++)
                        <svg class="w-5 h-5 {{ $s < $testimonial->rating ? 'text-[#FEBC11]' : 'text-white/80' }}" fill="currentColor" viewBox="0 0 20 20">
                ...
                <div>
                    ...
                    @if($testimonial->safariPackage)
                        <p class="text-white/80 text-sm mt-1">{{ $testimonial->safariPackage->translated('title') }}</p>
                    @endif
                </div>
            </div>
            @endforeach
