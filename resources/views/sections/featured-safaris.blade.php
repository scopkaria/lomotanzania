...
<section class="py-20 md:py-28 bg-white">
    <div class="max-w-7xl mx-auto px-6">
        ...
        <div class="text-center mb-14 reveal">
            <p class="text-xs font-semibold tracking-[0.3em] uppercase text-[#FEBC11] mb-3">{{ __('messages.explore_itineraries') }}</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="group bg-white rounded-xl shadow-sm overflow-hidden hover:-translate-y-1.5 hover:shadow-md transition-all duration-300 reveal" style="transition-delay: {{ ($loop->index + 1) * 100 }}ms;">
                ...
                <div class="p-6">
                    <div class="flex items-center gap-2 mb-3">
                        ...
                        <span class="text-xs font-semibold uppercase tracking-wider text-[#131414]/40">{{ $safari->duration ?? __('messages.multi_day') }}</span>
                    </div>
                    <h3 class="font-heading text-xl font-bold text-[#131414] mb-2">{{ $safari->translated('title') }}</h3>
                    <p class="text-sm text-[#131414]/50 leading-relaxed mb-5">{{ Str::limit($safari->translated('short_description'), 140) }}</p>
                    <a href="{{ route('safaris.show', $safari->slug) }}"
                       class="inline-block px-5 py-2.5 border-2 border-[#FEBC11] text-[#131414] text-xs font-bold uppercase tracking-wider rounded hover:bg-[#FEBC11] hover:text-[#131414] transition-all duration-300">
            ...
            <div class="col-span-full text-center py-12">
                <p class="text-[#131414]/40 text-sm">{{ __('messages.featured_coming_soon') }}</p>
            </div>
            @endforelse
        </div>
    </div>
