...
<head>
    ...
    <title>Admin Login — {{ optional($siteSetting ?? null)->site_name ?: 'Lomo Tanzania Safari' }}</title>
    ...
    <style>
        ...
        .login-hero {
            background: linear-gradient(135deg, rgba(8,51,33,0.8) 0%, rgba(19,20,20,0.7) 100%),
    ...
    <div class="hidden lg:flex lg:w-1/2 login-hero relative items-end p-12">
        <div class="relative z-10">
            <div class="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full px-4 py-1.5 mb-6">
                ...
                <span class="text-emerald-300 text-xs font-medium tracking-wider uppercase">Administration</span>
            </div>
            <h2 class="font-serif text-4xl text-white leading-tight mb-3">Admin<br>Dashboard</h2>
            <p class="text-white/60 text-sm max-w-sm">Manage safaris, bookings, content, and day-to-day operations.</p>
        </div>
    </div>
    ...
    <div class="flex-1 flex items-center justify-center px-6 py-12">
        <div class="w-full max-w-md">
            ...
            <div class="text-center mb-10">
                ...
                @else
                    <div class="w-14 h-14 rounded-2xl bg-brand-green flex items-center justify-center mx-auto mb-6">
                        <span class="text-white font-bold text-xl">L</span>
                    </div>
                @endif
                <h1 class="font-serif text-2xl text-brand-dark mb-1">Admin Login</h1>
                <p class="text-gray-400 text-sm">Sign in to manage your platform</p>
            </div>
                ...
                <form method="POST" action="{{ route('admin.login.store') }}" class="space-y-5">
                    ...
                    <div class="flex items-center justify-between">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="remember" class="w-4 h-4 rounded border-gray-300 text-brand-green focus:ring-brand-green/30">
                            <span class="text-sm text-gray-500">Remember me</span>
                        </label>
                        <a href="{{ route('password.request') }}" class="text-sm text-brand-green hover:text-brand-green/80 font-medium transition">Forgot password?</a>
                    </div>
