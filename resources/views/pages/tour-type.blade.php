...
<section class="relative h-[50vh] min-h-[380px] overflow-hidden">
    @if($tourType->featured_image)
        <img src="{{ asset('storage/' . $tourType->featured_image) }}"
             alt="{{ $tourType->translated('name') }}"
...
{{-- Description --}}
@if($tourType->translated('description'))
<section class="py-16 md:py-24 bg-white">
    <div class="max-w-4xl mx-auto px-6">
        <h2 class="font-heading text-2xl md:text-3xl font-bold text-brand-dark mb-6">{{ __('messages.about_type', ['name' => $tourType->translated('name')]) }}</h2>
        <div class="prose prose-lg max-w-none text-brand-dark/90 leading-relaxed">
            {!! nl2br(e($tourType->translated('description'))) !!}
        </div>
    </div>
</section>
@endif
...
{{-- Related Safaris --}}
<section class="py-16 md:py-24 {{ $tourType->translated('description') ? 'bg-brand-light' : 'bg-white' }}">
    <div class="max-w-7xl mx-auto px-6">
        ...
        <div class="text-center mb-14">
            ...
            <h2 class="font-heading text-3xl md:text-4xl font-bold text-brand-dark leading-tight">
                {{ __('messages.safaris_for_type', ['name' => $tourType->translated('name')]) }}
            </h2>
        </div>
        ...
        @else
            <div class="text-center py-12">
                <p class="text-brand-dark/90 text-sm">{{ __('messages.no_safaris_type') }}</p>
            </div>
        @endif
    </div>
</section>
